...
from controller import Robot, Keyboard, Motion, Display
import cv2 as cv
import numpy as np
...
class Nao (Robot):
    ...
    #from https://learncodebygaming.com/blog/hsv-color-range-thresholding
    ...
    def detectBall(self, camera, display):
        image = camera.getImage()
        #TODO
        width = camera.getWidth()
        height = camera.getHeight()
        #image_matrix = np.zeros((width, height, 4), dtype = "uint8")
        #image_matrix = cv.mat(Size(width, height), CV_8UC4);
        #image = image.astype(np.uint8)

        image = np.ndarray((width, height, 4), np.uint8, image)
        #print(image[0])
        #image = np.delete(image, 3, 2)
        
        rectangles = cascade.detectMultiScale(image)
        detected_image = self.draw_rectangles(image, rectangles)
        
        #detected_image = np.insert(detected_image, 3, 255, axis=2)
        #print(np.shape(detected_image))
        
        ir = display.imageNew(image.tobytes(), Display.BGRA, width, height)
        display.imagePaste(ir, 0, 0, False)
        display.imageDelete(ir)
        #cv.imshow('preview', detected_image)
    ...
    def run(self):
        ...
        display = super(Nao, self).getDevice('proc_im_display')
        #display.attachCamera(robot.cameraTop)
        
        
        
        while True:
            
            self.detectBall(self.cameraTop, display)
            key = self.keyboard.getKey()
